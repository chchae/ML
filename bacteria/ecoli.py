...
class MyDataFrame :
    def __init__( self, fname, nBits=1024, activity_criteria=0.5 ) :
    ...
    def ReadChEMBLCSV( self, fname ) :
        self.mydf = pd.read_csv( fname, usecols=['activity', 'smiles'], nrows=9999999 )
        ...
        if not set( [ 'PCHEMBL_VALUE', 'CANONICAL_SMILES' ] ).issubset( self.mydf.columns ) :
            self.mydf.rename( columns = { 'activity': 'PCHEMBL_VALUE', 'smiles': 'CANONICAL_SMILES' }, inplace=True, errors="raise" )
        return
...
def main() :
    nBits = 1024
    activity_criteria = 0.5
    frac_test = 0.3

    datafile = '../data/bacteria/ecoli.csv'
    # datafile = '../data/hERG/hERG_ChEMBL27.csv'
